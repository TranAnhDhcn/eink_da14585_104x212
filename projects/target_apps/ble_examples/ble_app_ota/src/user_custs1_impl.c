...
uint16_t indication_counter
    __SECTION_ZERO("retention_mem_area0"); //@RETENTION MEMORY
uint16_t non_db_val_counter
    __SECTION_ZERO("retention_mem_area0"); //@RETENTION MEMORY
...
ke_msg_id_t
    timer_used_min __SECTION_ZERO("retention_mem_area0"); //@RETENTION MEMORY
...
void do_display_update_with_analog_clock(void) {
  ...
  bool force_redraw =
      (last_update_time == 0 || (current_unix_time - last_update_time) > 3600 ||
       g_tm.tm_min == 0);
  ...
  case DISPLAY_MODE_CALENDAR_ANALOG:
    draw_calendar_with_analog_clock(current_unix_time,
                                    force_redraw || minute_changed);
    break;

  case DISPLAY_MODE_FABRIC_RECORD:
    draw_fabric_record_page(&current_fabric_record,
                            force_redraw || minute_changed);
    break;
  ...
  //     EPD_DrawUTF8(189, 3, 0, buf, EPD_ASCII_Font8, 0, WHITE, BLACK);
  // } else {
  //     EPD_DrawUTF8(189, 3, 0, buf, EPD_ASCII_Font8, 0, BLACK, WHITE);
  // }

  // Hiển thị kết nối BLE (Sửa Y từ 150 về 1)
  // if (isconnected == 1)
  // {
  //     EPD_DrawUTF8(200, 1, 0, "B", EPD_ASCII_Font8, 0, BLACK, WHITE);
  // }
...
void do_min_work_with_analog_clock(void) {
  timer_used_min = app_easy_timer(APP_PERIPHERAL_CTRL_TIMER_DELAY_MINUTES,
                                  do_min_work_with_analog_clock);
...
void user_svc1_ctrl_wr_ind_handler(ke_msg_id_t const msgid,
  ...
  if (val != CUSTS1_CP_ADC_VAL1_DISABLE) {
    timer_used = app_easy_timer(APP_PERIPHERAL_CTRL_TIMER_DELAY,
...
void bls_att_pushNotifyData(uint16_t attHandle, uint8_t *p, uint8_t len) {
  struct custs1_val_ntf_ind_req *req = KE_MSG_ALLOC_DYN(
      CUSTS1_VAL_NTF_REQ, prf_get_task_from_id(TASK_ID_CUSTS1), TASK_APP,
      custs1_val_ntf_ind_req, DEF_SVC1_ADC_VAL_1_CHAR_LEN);
...

{
  ...
  // Lệnh 0x02 và 0x03: set IMAGE mode và ghi data người dùng - bypass guard
  // step
  if (param->value[0] == 0x02) {
    ASSERT_MIN_LEN(payload_len, 3);
    byte_pos = payload[1] << 8 | payload[2];
    // Set IMAGE mode ngay khi bắt đầu upload để minute timer không overwrite
    // epd_buffer
...
/**
 * @brief Hàm xử lý lệnh ghi cổng nối tiếp (Custom Service 2) đã sửa đổi, thêm
 * chuyển đổi chế độ đồng hồ kim
 */
void user_svc2_wr_ind_handler(ke_msg_id_t const msgid,
  ...
  if ((param->value[0] == 0xDD) && (param->length >= 5)) {
    ...
    timer_used_min =
        app_easy_timer(time_offset * 100, do_min_work_with_analog_clock);
...
void user_svc1_long_val_ntf_cfm_handler(
    ke_msg_id_t const msgid, struct custs1_val_write_ind const *param,
    ke_task_id_t const dest_id, ke_task_id_t const src_id) {}
...
void user_svc1_adc_val_1_ntf_cfm_handler(
    ke_msg_id_t const msgid, struct custs1_val_write_ind const *param,
    ke_task_id_t const dest_id, ke_task_id_t const src_id) {}
...
void user_svc1_indicateable_cfg_ind_handler(
    ke_msg_id_t const msgid, struct custs1_val_write_ind const *param,
    ke_task_id_t const dest_id, ke_task_id_t const src_id) {}

void user_svc1_indicateable_ind_cfm_handler(
    ke_msg_id_t const msgid, struct custs1_val_write_ind const *param,
    ke_task_id_t const dest_id, ke_task_id_t const src_id) {}
...
void app_adcval1_timer_cb_handler() {
  ...
  if (ke_state_get(TASK_APP) == APP_CONNECTED) {
    // Set it once again until Stop command is received in Control
    // Characteristic
    timer_used = app_easy_timer(APP_PERIPHERAL_CTRL_TIMER_DELAY,
                                app_adcval1_timer_cb_handler);
  }
